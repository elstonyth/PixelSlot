...
// live /claw card (− 1 + MAX stepper + Open). Open links to the pack's
// claw-machine detail page: logged-out visitors can view the machine and use the
// free demo spin there with NO login — only a real open/claim is auth-gated.
// ---------------------------------------------------------------------------
...
function PackCard({ pack, icon }: { pack: Pack; icon: string }) {
  ...
  return (
    <div className="group relative flex h-full flex-col rounded-2xl border border-white/10 bg-white/5 p-3 shadow-[0_4px_20px_rgba(0,0,0,0.25)] transition-colors duration-300 hover:border-white/20">
      ...
      {/* Open → the pack's claw-machine detail page. The free demo spin there is
          open to everyone; only a real open/claim is auth-gated. */}
      <Link
        href={`/claw/${pack.id}`}
