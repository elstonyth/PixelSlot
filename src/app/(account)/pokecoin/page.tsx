...
export default function PokeCoinPage() {
  return (
    <>
      <AccountHeader title="PokéCoin" sub="Earn coins on every purchase and spend them across Pokenic." />
      <div className="mb-5 flex items-center gap-4 rounded-2xl border border-white/10 bg-gradient-to-br from-amber-500/10 to-transparent p-6">
        ...
        <div>
          ...
          <p className="font-heading text-3xl font-bold text-white">{num(48250)} <span className="text-lg text-amber-300">PKC</span></p>
        </div>
      ...
      <MockTable
        ...
        rows={TX.map((t) => [t[0], t[1], <Badge key="a" tone={t[3]}>{t[2]} PKC</Badge>])}
      />
      <DemoNote />
    </>
  );
