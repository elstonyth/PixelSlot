...
export function Panel({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`rounded-2xl border border-white/10 bg-white/[0.03] p-5 sm:p-6 ${className}`}>{children}</div>;
}
        ...
        <tbody>
          {rows.map((r, i) => (
            <tr key={i} className="border-b border-white/5 last:border-0 hover:bg-white/[0.02]">
              {r.map((c, j) => (
                <td key={j} className="whitespace-nowrap px-4 py-3 text-white/80">{c}</td>
              ))}
            </tr>
          ))}
        </tbody>
...
export function DemoNote() {
  return <p className="mt-5 text-[11px] text-white/35">Demo only — this account area connects to the backend in a later phase.</p>;
}
