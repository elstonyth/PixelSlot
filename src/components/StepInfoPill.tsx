...
export default function StepInfoPill({
  ...
  if (variant === "packs") {
    return (
      <Link href="/claw" className={cn(base, "group/pill hover:border-white/20 hover:bg-white/[0.07]")}>
        {/* eslint-disable-next-line @next/next/no-img-element */}
        <img src="/images/claw/rookie-pack-icon.webp" alt="" aria-hidden className="h-7 w-7 shrink-0 object-contain" />
        {Label}
        <ArrowRight className="h-4 w-4 shrink-0 text-white/50 transition-transform duration-300 group-hover/pill:translate-x-0.5 group-hover/pill:text-white" aria-hidden />
      </Link>
    );
  }
      ...
      {open && createPortal(
        ...
        >
          ...
          <div className="relative z-10 w-full max-w-[480px] rounded-2xl border border-white/10 bg-neutral-900 p-6 shadow-2xl motion-safe:animate-[modalIn_0.25s_ease-out] sm:p-7">
            ...
            <h3 id="buyback-title" className="font-heading text-xl font-bold text-white sm:text-2xl">
              85-90% Instant Buyback
            </h3>
            <p className="mt-3 text-[13px] leading-relaxed text-white/60 sm:text-sm">
              Every card you pull has a guaranteed buyback price set at 85-90% of its Alt
              Fair Market Value. Pull a card you don&apos;t want? Sell it back instantly
              with one tap.
            </p>
            ...
            <div className="mt-6 grid grid-cols-3 gap-3">
              <MiniStep label="Open a pack">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/images/claw/platinum-pack-icon.webp" alt="" className="h-12 w-auto object-contain" />
              </MiniStep>
              <MiniStep label="Pull a card">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src="/home/hero/slabs/pokemon1.webp" alt="" className="h-12 w-auto object-contain" />
              </MiniStep>
              <MiniStep label="Sell instantly">
                <span className="flex h-12 w-12 items-center justify-center rounded-full bg-emerald-500/15">
                  <DollarSign className="h-6 w-6 text-emerald-400" aria-hidden />
                </span>
              </MiniStep>
            </div>
            ...
            <div className="mt-4 rounded-xl border border-white/10 bg-white/[0.03] px-4 py-3">
              ...
              <div className="mt-2 flex items-center justify-between text-sm">
                ...
                <span className="font-semibold text-emerald-400">$85.00 - $90.00</span>
              </div>
            </div>

            <p className="mt-4 text-[12px] leading-relaxed text-white/45">
              Prices are based on Alt Fair Market Value and updated in real time. Keep your
              grails, sell the rest.
            </p>
...
function MiniStep({ label, children }: { label: string; children: React.ReactNode }) {
  return (
