...
export class Migration20260629132054 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`alter table if exists "achievement_member_state" drop constraint if exists "achievement_member_state_customer_id_unique";`);
    this.addSql(`alter table if exists "achievement_grant" drop constraint if exists "achievement_grant_unique";`);
    this.addSql(`alter table if exists "achievement_def" drop constraint if exists "achievement_def_key_unique";`);
    ...
    this.addSql(`alter table if exists "admin_action_audit" add constraint "admin_action_audit_entity_type_check" check("entity_type" in ('customer', 'commission', 'rewards_settings', 'credit', 'reward_pool'));`);
    this.addSql(`alter table if exists "admin_action_audit" add constraint "admin_action_audit_action_check" check("action" in ('freeze', 'unfreeze', 'reverse_commission', 'suspend_commission', 'unsuspend_commission', 'adjust_credit', 'edit_rewards_settings', 'edit_reward_pool'));`);
  }
  ...
  override async down(): Promise<void> {
    ...
    this.addSql(`alter table if exists "admin_action_audit" add constraint "admin_action_audit_entity_type_check" check("entity_type" in ('customer', 'commission', 'rewards_settings', 'credit'));`);
    this.addSql(`alter table if exists "admin_action_audit" add constraint "admin_action_audit_action_check" check("action" in ('freeze', 'unfreeze', 'reverse_commission', 'suspend_commission', 'unsuspend_commission', 'adjust_credit', 'edit_rewards_settings'));`);
  }
