  ...
  testSuite: ({ getContainer }) => {
    ...
    describe("team override inside settleOpen", () => {
      it("pays direct + decaying overrides up the chain (depth-keyed generations)", async () => {
        ...
        const s1 = "cus_ov_s1", s2 = "cus_ov_s2", s3 = "cus_ov_s3";
        ...
        await packs.mutateCreditAtomic({
          customerId: recruit, amount: 100, reason: "topup", reference: "mock_ov",
        });
        // s1 = L1 (spend 0) -> 1% -> direct 100 sen. Overrides: s2 = 20% of 100 = 20,
        // s3 = 20% of 20 = 4; depth-4 = 4*0.2 = 0.8 < 1 sen -> none.
        const r = await packs.settleOpen({
          customerId: recruit, amount: -100, sourceTransactionId: "open_ov_1",
        });
        ...
        const comms = (
          await packs.listCommissions({ source_transaction_id: "open_ov_1" }, { take: 10 })
      ...
      it("is idempotent: replaying the open_id rejects and pays each generation once", async () => {
        const packs = getContainer().resolve<PacksModuleService>(PACKS_MODULE);
        await seedLadder(packs);
        const recruit = "cus_ovi_recruit", s1 = "cus_ovi_s1", s2 = "cus_ovi_s2";
        await packs.linkSponsor({ recruitId: recruit, sponsorId: s1 });
        await packs.linkSponsor({ recruitId: s1, sponsorId: s2 });
        await packs.mutateCreditAtomic({
          customerId: recruit, amount: 200, reason: "topup", reference: "mock_ovi",
        });
        await packs.settleOpen({ customerId: recruit, amount: -100, sourceTransactionId: "open_ovi" });
        // Replaying the SAME open_id must reject and roll back the whole open.
        await expect(
          packs.settleOpen({ customerId: recruit, amount: -100, sourceTransactionId: "open_ovi" }),
        ).rejects.toThrow();
        const comms = await packs.listCommissions(
          { source_transaction_id: "open_ovi" }, { take: 10 },
        );
