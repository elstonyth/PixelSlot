  ...
  for (const type of ["pointerdown", "pointermove", "pointerup", "click"]) {
    document.addEventListener(
      ...
      (e) => {
        ...
        const t = e.target;
        window.__log.push(`${type} -> ${t.tagName}.${("" + (t.className || "")).toString().slice(0, 40)} @${Math.round(e.clientX)},${Math.round(e.clientY)} buttons=${e.buttons}`);
      },
      true,
    );
  }
  const cont = [...document.querySelectorAll("div")].find((d) => ("" + d.className).includes("cursor-grab"));
  window.__cont = cont;
  window.__hit = document.elementFromPoint(720, 450);
  window.__contRect = cont ? JSON.stringify(cont.getBoundingClientRect()) : "no container";
});
console.log("container rect:", await page.evaluate(() => window.__contRect));
console.log("elementFromPoint(720,450):", await page.evaluate(() => { const e = window.__hit; return e ? `${e.tagName}.${("" + e.className).slice(0, 60)}` : "none"; }));
...
const dur = await page.evaluate(() => {
  ...
  return el ? el.style.transform || getComputedStyle(el).transform.slice(0, 60) : "no cyl";
});
await page.mouse.up();
await page.waitForTimeout(900);
...
console.log(await page.evaluate(() => (document.body.innerText.match(/TAP TO [A-Z ]+|SHUFFLE|CATEGORY|Continue/gi) || []).join(" | ")));
await browser.close();
