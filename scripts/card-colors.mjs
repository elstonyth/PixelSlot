...
for (const [name, src] of themes) {
  const rgb = await p.evaluate(async (src) => {
    ...
    const ctx = c.getContext("2d"); ctx.drawImage(img, 0, 0, 40, 60);
    const d = ctx.getImageData(0, 0, 40, 60).data;
    let r=0,g=0,bl=0,n=0;
    for (let i=0;i<d.length;i+=4){const R=d[i],G=d[i+1],B=d[i+2];const mx=Math.max(R,G,B),mn=Math.min(R,G,B);const sat=mx-mn;
      // weight by saturation so we capture the vivid hue, not the white/grey packaging
      const w = sat*sat; r+=R*w; g+=G*w; bl+=B*w; n+=w;}
