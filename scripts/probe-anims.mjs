...
const ROUTES = ["/", "/claw", "/pack-party", "/activity", "/store", "/marketplace"];
...
const EXTRACT = () => {
  ...
  for (const el of document.querySelectorAll("*")) {
    ...
    if (!map[key]) map[key] = { count: 0, duration: s.animationDuration, sample: (el.tagName.toLowerCase() + "." + (el.className?.toString().split(" ")[0] || "")).slice(0, 40) };
    map[key].count++;
  }
  return map;
};
...
for (const route of ROUTES) {
  ...
  try {
    ...
    for (const k of keys) console.log(`  ${k.padEnd(22)} x${m[k].count}  ${m[k].duration}  e.g. ${m[k].sample}`);
