...
const ROUTES = ["/", "/claw", "/claw/pokemon-rookie", "/pack-party", "/marketplace", "/leaderboard", "/how-it-works", "/about", "/contact", "/activity", "/pokemon/generation/1", "/series", "/card/charizard-ex-scarlet-violet-151-1", "/profile/FightingProdigy3098", "/login", "/signup", "/settings", "/orders", "/earnings", "/borrow-lend", "/pokecoin", "/accelerate-claim", "/lucky-draw", "/roulette", "/repacks", "/clawmaker", "/store", "/free", "/fairness", "/social", "/merchants", "/airdrop", "/launchpad/fwog", "/30th"];
...
for (const route of ROUTES) {
  const name = route === "/" ? "home" : route.replace(/^\//, "").replace(/\//g, "_").replace(/-/g, "");
  for (const w of WIDTHS) {
    ...
    try {
      ...
      report.push(`${name.padEnd(12)} ${String(w).padStart(4)}px  overflow=${String(m.overflow).padStart(4)}  imgs=${m.imgTotal}${flag}${bflag}`);
      if (m.broken.length) m.broken.forEach((b) => report.push(`      broken: ...${b}`));
      if (SHOT_AT.has(w)) {
