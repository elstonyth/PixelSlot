...
const EXTRACT = () => {
  ...
  const P = ["backgroundColor","color","borderRadius","padding","fontSize","fontWeight","border","boxShadow","backgroundImage"];

  const byText = (re, tags = ["button", "a"]) => {
    const els = [...document.querySelectorAll(tags.join(","))].filter((e) => re.test(e.textContent.trim()));
  ...
  return {
    openPacks: openPacks ? { text: openPacks.textContent.trim(), tag: openPacks.tagName.toLowerCase(), styles: cs(openPacks, P), rect: rct(openPacks) } : "NOT FOUND",
    howItWorks: howItWorks ? { text: howItWorks.textContent.trim(), tag: howItWorks.tagName.toLowerCase(), styles: cs(howItWorks, P), rect: rct(howItWorks) } : "NOT FOUND",
  };
};
