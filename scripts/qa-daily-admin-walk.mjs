...
if (run('3a')) {
  ...
  const r1 = rows.nth(0);
  check(
    (await r1.locator('button[role="combobox"]').first().innerText()).includes(
      'Credit',
    ),
  ...
  check(
    (await r1.locator('button[role="switch"]').getAttribute('aria-checked')) ===
      'true',
...
if (run('3b')) {
  // --- 3b: vouchers ladder ---
  const ladder = await fetch(`${API}/admin/daily-rewards/vouchers`, {
    headers: AH,
  }).then((r) => r.json());
