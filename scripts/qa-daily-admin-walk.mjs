...
const AH = { 'Content-Type': 'application/json', Authorization: `Bearer ${auth.token}` };
...
const openDailyRewards = async () => {
  await page.goto(`${BASE}/daily-rewards`, { waitUntil: 'domcontentloaded', timeout: 15000 });
  await page.waitForSelector('h2:has-text("Daily Rewards")', { timeout: 30000 });
};
...
if (run('3c')) {
  ...
  check(/Daily Rewards/.test(bodyText), '3c sidebar/page shows "Daily Rewards"');
  check(!/Reward Pools/i.test(bodyText), '3c "Reward Pools" absent from the rendered app');
...
if (run('3a')) {
  ...
  await page.locator('label:has(span:text("Enabled")) button[role="switch"]').click();
  ...
  await page.screenshot({ path: 'docs/research/task15-admin-box-a-authored.png' });

  // Round-trip: reload, tier a is the default selection again.
  await page.reload({ waitUntil: 'domcontentloaded', timeout: 15000 });
  await page.waitForSelector('button:has-text("LV 1–9")', { timeout: 30000 });
  await page.waitForSelector('table tbody tr', { timeout: 20000 });
  check((await rows.count()) === 2, '3a round-trip: 2 prize rows after reload');
  const r1 = rows.nth(0);
  check(
    (await r1.locator('button[role="combobox"]').first().innerText()).includes('Credit'),
  ...
  check(
    (await r1.locator('button[role="switch"]').getAttribute('aria-checked')) === 'true',
  ...
  check(
    (await rows.nth(1).locator('button[role="combobox"]').first().innerText()).includes('Nothing'),
  ...
  await page.screenshot({ path: 'docs/research/task15-admin-box-a-roundtrip.png' });
}

if (run('3b')) {
  // --- 3b: vouchers ladder ---
  const ladder = await fetch(`${API}/admin/daily-rewards/vouchers`, { headers: AH }).then((r) =>
    r.json(),
  );
  ...
  await page.screenshot({ path: 'docs/research/task15-admin-vouchers-seeded.png' });
  ...
  if (overCeiling.length > 0) {
    const errText = await page.locator('text=/exceeds the RM 10,000 ceiling/').count();
    check(errText > 0, '3b UI shows the RM 10,000 ceiling error for the seeded L90/L100 values');
    check(
      await page.locator('button:text-is("Save")').isDisabled(),
      '3b Save is disabled while over-ceiling rows exist (pre-existing data!)',
    );
    await page.screenshot({ path: 'docs/research/task15-admin-vouchers-ceiling-block.png' });
  }
  ...
  await saveWithReason('task-15 e2e verify: bump 7-9 to RM 7 (+cap L90/L100 to pass UI ceiling)');
  ...
  await page.screenshot({ path: 'docs/research/task15-admin-vouchers-edited.png' });
  ...
  const restore = await fetch(`${API}/admin/daily-rewards/vouchers`, {
    method: 'POST',
    headers: AH,
    body: JSON.stringify({
      ranges: originalRanges,
      reason: 'task-15 e2e verify: restore original ladder (L90/L100 back over UI ceiling)',
    }),
  });
  check(restore.ok, `3b API restore of the original ladder (HTTP ${restore.status})`);

  await openDailyRewards();
  await openVouchersTab();
  check(same(await readRanges(), originalRanges), '3b restored: ladder back to original values');
  await page.screenshot({ path: 'docs/research/task15-admin-vouchers-restored.png' });
}
