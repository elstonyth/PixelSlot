...
const OVERRIDE = {
  "black-pack-jjnfuk":    { box: [0.370, 0.628, 0.112, 0.166], font: 50 },
  "legend-pack-1dpaec":   { box: [0.375, 0.628, 0.112, 0.166], font: 38 },
  "modern-grails-noafw0": { box: [0.288, 0.578, 0.115, 0.170], font: 38 },
  "pro-soccer-pack":      { box: [0.378, 0.606, 0.115, 0.170], font: 50 },
};

// back up originals once; then read sources from the backups
const inputs = {};
for (const base of Object.keys(OVERRIDE)) {
  const src = `${DIR}/${base}-machine.webp`, bak = `${DIR}/${base}-machine-src.webp`;
  try { await access(bak); } catch { await copyFile(src, bak); }
  inputs[base] = "data:image/webp;base64," + (await readFile(bak)).toString("base64");
}
...
const results = await page.evaluate(async ({ OVERRIDE, inputs }) => {
  ...
  for (const [base, cfg] of Object.entries(OVERRIDE)) {
    ...
    const x0 = Math.round(cfg.box[0] * W), x1 = Math.round(cfg.box[1] * W), y0 = Math.round(cfg.box[2] * H), y1 = Math.round(cfg.box[3] * H);
    ...
    const isText = (x, y) => { const i = (y * W + x) * 4; return Math.min(d[i], d[i + 1], d[i + 2]) > 150; };
...
for (const [base, r] of Object.entries(results)) {
  if (r.ok) { await writeFile(`${DIR}/${base}-machine.webp`, Buffer.from(r.data.split(",")[1], "base64")); console.log(`${base.padEnd(24)} edited fs=${r.fs}`); }
