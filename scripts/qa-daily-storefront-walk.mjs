...
const pubRes = await fetch(`${API}/auth/user/emailpass`, {
  method: 'POST',
...
const apiDaily = () =>
  fetch(`${API}/store/daily`, { headers: CH }).then((r) => r.json());
...
for (let i = 0; i < drawsLeft; i++) {
  await openBox.click();
  const dialog = page.locator('[role="dialog"][aria-label="Daily box reveal"]');
