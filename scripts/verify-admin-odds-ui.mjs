...
await page.fill('input[type="email"], input[name="email"]', "admin@pokenic.local");
await page.fill('input[type="password"], input[name="password"]', "pokenicadmin2026");
...
const listHeading = await page.getByText("Gacha Packs").first().isVisible().catch(() => false);
...
await page.screenshot({ path: `${OUT}/02-admin-packs-list.png`, fullPage: true });
...
const editorRows = await page.locator("table tbody tr").count().catch(() => 0);
ok("editor_16_rows", editorRows === 16, `rows ${editorRows}`);
const hasAfterSave = await page.getByText("After save").first().isVisible().catch(() => false);
...
await page.screenshot({ path: `${OUT}/03-admin-odds-editor.png`, fullPage: true });
...
const interaction = await page.evaluate(async () => {
  ...
  const saveBtn = [...document.querySelectorAll("button")].find((b) => /Save win rates/i.test(b.innerText));
...
await page.screenshot({ path: `${OUT}/05-admin-odds-saved.png`, fullPage: true });
...
const after = await (await fetch(`${API}/admin/packs/${SLUG}/odds`, {
  headers: { Authorization: `Bearer ${token}` },
})).json();
const top = after.odds[0];
ok("persisted_via_ui", top.locked === true && top.pct === 40, JSON.stringify({ id: top.card_id, pct: top.pct, locked: top.locked }));
