...
const save = (dir, name, data) => {
  ...
  console.log("WROTE", `${dir}/${name}`, Array.isArray(data?.frames) ? `${data.frames.length} frames` : "");
};
...
const installRecorder = () =>
  page.evaluate(() => {
    const sig = (el) => {
      ...
      const src = (img?.currentSrc || img?.src || "").split("/").pop()?.split("?")[0]?.slice(-26) || "";
    ...
    const findOverlay = () =>
      [...document.querySelectorAll("div")].find((d) => {
        ...
        return s.position === "fixed" && d.getBoundingClientRect().width > innerWidth * 0.8 && d.querySelector("img,button");
      });
    const MODES = {
      ...
      hero() {
        ...
        [...document.querySelectorAll("img")].forEach((im) => {
          ...
          if (!(r.top < 560 && r.bottom > 40 && r.x > 560 && r.width > 50)) return;
          let w = im.parentElement, d = 0;
          while (w && d < 4 && getComputedStyle(w).transform === "none" && getComputedStyle(w).opacity === "1") { w = w.parentElement; d++; }
          if (w) wraps.add(w);
        });
        return [...wraps];
      },
      ...
      overlay() {
        ...
        return [...o.querySelectorAll("*")]
          .filter((el) => {
            ...
            return cs.transform !== "none" || +cs.opacity < 1 || cs.animationName !== "none" || cs.translate !== "none" || cs.scale !== "none" || cs.rotate !== "none";
          })
          .slice(0, 36);
      },
      ...
      cylinder() {
        const o = findOverlay();
        if (!o) return [];
        const cyl = [...o.querySelectorAll("*")].find((el) => getComputedStyle(el).transformStyle === "preserve-3d");
        return cyl ? [cyl] : [];
      },
    ...
    window.__recStart = (mode, arg) => {
      const data = { mode, arg: arg || null, t0: performance.now(), frames: [], stopped: false };
      window.__recData = data;
      const tick = () => {
        if (data.stopped) return;
        const t = +(performance.now() - data.t0).toFixed(1);
        let els = [];
        try { els = MODES[mode](arg) || []; } catch { /* targets gone mid-flow */ }
        const fr = { t, n: [] };
        for (const el of els) {
          try {
            const cs = getComputedStyle(el);
            const r = el.getBoundingClientRect();
            const rec = { s: sig(el), tr: cs.transform, o: +(+cs.opacity).toFixed(3), x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height) };
            ...
            if (cs.animationName !== "none") rec.an = `${cs.animationName.slice(0, 28)}|${cs.animationDuration}|${cs.animationTimingFunction.slice(0, 40)}`;
            fr.n.push(rec);
          } catch { /* detached */ }
        }
        if (mode === "overlay" && window.__overlayEl) {
          fr.txt = (window.__overlayEl.innerText || "").replace(/\s+/g, " ").trim().slice(0, 90);
        }
        data.frames.push(fr);
...
const settleImages = async () => {
  for (let i = 0; i < 25; i++) {
    if (await page.evaluate(() => document.querySelectorAll("img").length > 5)) break;
  ...
  const read = () =>
    el.evaluate((node) => {
      const grab = (e) => {
        ...
        return { transform: cs.transform, translate: cs.translate, scale: cs.scale, rotate: cs.rotate, boxShadow: cs.boxShadow.slice(0, 90), filter: cs.filter.slice(0, 60), opacity: cs.opacity, transition: cs.transition.slice(0, 160), borderColor: cs.borderColor, outline: cs.outline.slice(0, 60) };
      };
      return { self: grab(node), kids: [...node.querySelectorAll(":scope > *, :scope img")].slice(0, 5).map((k) => ({ tag: k.tagName, cls: ("" + k.className).slice(0, 40), ...grab(k) })) };
    });
...
// ================================ PHASE A: HOME ==============================
try {
  await page.goto("https://www.phygitals.com/", { waitUntil: "domcontentloaded", timeout: 60000 });
  await settleImages();
  ...
  const scrollerInfo = await page.evaluate(() => {
    ...
    const cands = [...sc.querySelectorAll("section,div")]
      .filter((el) => {
        const cs = getComputedStyle(el);
        const r = el.getBoundingClientRect();
        return r.top > innerHeight && r.top < innerHeight + 3000 && r.height > 120 && (+cs.opacity < 0.05 || (cs.transform !== "none" && cs.transform !== "matrix(1, 0, 0, 1, 0, 0)"));
      })
      .slice(0, 6)
      .map((el, i) => {
        el.setAttribute("data-recon", "entry-" + i);
        const cs = getComputedStyle(el);
        return { i, cls: ("" + el.className).slice(0, 60), top: Math.round(el.getBoundingClientRect().top), opacity: cs.opacity, transform: cs.transform, transition: cs.transition.slice(0, 140) };
      });
    return { found: true, scTag: sc.tagName + "." + ("" + sc.className).slice(0, 40), cands };
  });
  console.log("scroll-entry candidates:", JSON.stringify(scrollerInfo).slice(0, 400));
  if (scrollerInfo.found && scrollerInfo.cands.length) {
    ...
    await page.evaluate(() => { window.__scroller.scrollTop += innerHeight * 0.45; });
    await page.waitForTimeout(2600);
    const entry = await page.evaluate(() => window.__recStop());
    entry.candidate = target;
    save(OUT_PAGE, "entry-curve.json", entry);
  } else {
    save(OUT_PAGE, "entry-curve.json", { found: false, note: "no opacity-0 below-fold candidates", scrollerInfo });
  }

  // A3 — hover probe on a home card (a link with an image, mid-page)
  await page.evaluate(() => { window.__scroller && (window.__scroller.scrollTop = 900); });
  await page.waitForTimeout(900);
  const homeCard = page.locator("a:has(img)").filter({ hasNot: page.locator("header a") }).nth(3);
...
// ================================ PHASE B: /claw =============================
try {
  await page.goto("https://www.phygitals.com/claw", { waitUntil: "domcontentloaded", timeout: 60000 });
  await settleImages();
  await installRecorder();

  const catalog = await page.evaluate(() => {
    const rows = [...document.querySelectorAll("div,ul")].filter((el) => el.scrollWidth > el.clientWidth * 1.15 && el.querySelectorAll(":scope img").length >= 3);
    return rows.slice(0, 4).map((row) => {
      ...
      const near = row.parentElement ? [...row.parentElement.querySelectorAll(":scope > button, :scope > div > button")].map((b) => b.innerText.trim() || b.getAttribute("aria-label") || "svg-btn").slice(0, 4) : [];
      return {
        cls: ("" + row.className).slice(0, 80),
        overflowX: cs.overflowX, scrollSnapType: cs.scrollSnapType, scrollBehavior: cs.scrollBehavior,
        gap: cs.gap, cursor: cs.cursor, scrollWidth: row.scrollWidth, clientWidth: row.clientWidth,
  ...
  const packCard = page.locator("a:has(img)").filter({ has: page.locator("img") }).nth(4);
...
try {
  await page.goto("https://www.phygitals.com/claw/black-pack", { waitUntil: "domcontentloaded", timeout: 60000 });
  await settleImages();
  await installRecorder();
  ...
  save(OUT_PACK, "cylinder-drag.json", await page.evaluate(() => window.__recStop()));
  ...
  if (await shuffle.isVisible().catch(() => false)) {
    ...
    save(OUT_PACK, "cylinder-shuffle.json", await page.evaluate(() => window.__recStop()));
  }
  ...
  for (let i = 0; i < 34; i++) {
    await page.screenshot({ path: `${OUT_PACK}/shot-${String(i).padStart(2, "0")}.png` }).catch(() => {});
    await page.waitForTimeout(400);
  }
  save(OUT_PACK, "reveal-track.json", await page.evaluate(() => window.__recStop()));
  ...
  const finalDom = await page.evaluate(() => {
    ...
    const grab = (e) => {
      ...
      return { tag: e.tagName, cls: ("" + e.className).slice(0, 70), x: Math.round(r.x), y: Math.round(r.y), w: Math.round(r.width), h: Math.round(r.height), fontSize: cs.fontSize, fontFamily: cs.fontFamily.split(",")[0], fontWeight: cs.fontWeight, color: cs.color, bg: cs.backgroundColor, radius: cs.borderRadius, border: cs.border.slice(0, 60), shadow: cs.boxShadow.slice(0, 90), transform: cs.transform.slice(0, 80), letterSpacing: cs.letterSpacing, textTransform: cs.textTransform, text: (e.innerText || "").replace(/\s+/g, " ").slice(0, 60) };
    };
    return {
      found: true,
      text: (o.innerText || "").replace(/\s+/g, " ").slice(0, 300),
      els: [...o.querySelectorAll("*")].filter((e) => { const r = e.getBoundingClientRect(); return r.width > 14 && r.height > 10; }).slice(0, 80).map(grab),
      animations: document.getAnimations().slice(0, 20).map((a) => ({ type: a.constructor.name, name: a.animationName || a.id || "", state: a.playState, timing: a.effect?.getTiming ? { duration: a.effect.getTiming().duration, delay: a.effect.getTiming().delay, easing: a.effect.getTiming().easing, iterations: a.effect.getTiming().iterations } : null })),
    };
  });
