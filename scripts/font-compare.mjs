...
const origB = "data:image/png;base64," + (await readFile("docs/research/packdetail/lama-in/legend-pack.png")).toString("base64");
const CANDIDATES = ["Poppins", "Montserrat", "Outfit", "Urbanist", "Nunito", "Quicksand", "Baloo 2", "Comfortaa", "DM Sans", "Manrope"];
const fams = CANDIDATES.map((c) => `family=${c.replace(/ /g, "+")}:wght@600;700`).join("&");
...
await page.screenshot({ path: "docs/research/packdetail/fontcmp.png", fullPage: true });
await browser.close();
