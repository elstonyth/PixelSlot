...
const EXTRACT = () => {
  ...
  for (const el of all) {
    ...
    const rects = kids.map((k) => k.getBoundingClientRect()).filter((r) => r.width > 80 && r.height > 80);
    if (rects.length < 4) continue;
    const widths = rects.map((r) => Math.round(r.width));
    const wmode = widths.sort((a, b) => widths.filter(x=>x===a).length - widths.filter(x=>x===b).length).pop();
    const sameW = rects.filter((r) => Math.abs(Math.round(r.width) - wmode) < 4).length;
    const score = sameW; // more equal-width children = more grid-like
    if (score > bestScore && sameW >= 4) { bestScore = score; best = { el, rects, kids }; }
  }
  if (!best) return "NO GRID";
  const { el, rects } = best;
  // columns = number of children sharing the minimum top (first row)
  const minTop = Math.min(...rects.map((r) => r.top));
  const firstRow = rects.filter((r) => Math.abs(r.top - minTop) < 8).sort((a, b) => a.left - b.left);
  const cols = firstRow.length;
  const cardW = Math.round(firstRow[0].width);
  const gapX = firstRow.length > 1 ? Math.round(firstRow[1].left - firstRow[0].right) : null;
  return {
    container: cs(el, ["display", "gridTemplateColumns", "gap", "columnGap", "rowGap", "padding", "maxWidth", "width"]),
    containerRect: { w: Math.round(el.getBoundingClientRect().width), left: Math.round(el.getBoundingClientRect().left) },
    columns: cols,
    cardWidth: cardW,
    gapX,
...
for (const route of ROUTES) {
  ...
  for (const [site, origin] of Object.entries(ORIGIN)) {
    ...
    } catch (e) { out[route][site] = { error: e.message }; }
    await ctx.close();
  }
}
await browser.close();
