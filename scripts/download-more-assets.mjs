...
import { createWriteStream, existsSync, statSync, mkdirSync, writeFileSync } from "node:fs";
...
async function dl(url, dest) {
  ...
  try {
    const r = await fetch(url, { headers: { "User-Agent": "Mozilla/5.0", Referer: "https://www.phygitals.com/" } });
...
for (let i = 1; i <= 160; i += 8) {
  ...
  for (let n = i; n < i + 8 && n <= 160; n++) {
    batch.push(
      dl(`https://www.phygitals.com/images/pfps/pfp-${n}.webp`, `public/images/pfps/pfp-${n}.webp`).then((ok) => ok && pfpIds.push(n)),
    );
  }
  await Promise.all(batch);
}
console.log(`pfps available: ${pfpIds.sort((a, b) => a - b).length} ->`, pfpIds.sort((a, b) => a - b).join(","));
...
for (const route of ["/", "/pack-party"]) {
  try {
    await page.goto("https://www.phygitals.com" + route, { waitUntil: "networkidle", timeout: 40000 }).catch(() => {});
    await page.waitForTimeout(2500);
    const found = await page.evaluate(() =>
      [...document.querySelectorAll("img")]
        .map((i) => i.currentSrc || i.src)
        .filter((s) => /cdn-cgi\/image|img\.phygitals\.com|arweave\.net/.test(s)),
    );
...
writeFileSync("docs/research/harvested-cards.json", JSON.stringify({ pfps: pfpIds.sort((a, b) => a - b), cards: manifest }, null, 2));
console.log(`downloaded ${manifest.length} card images -> public/cdn/cards/h-*.webp`);
