...
const adminTok = (await (await fetch(`${API}/auth/user/emailpass`, {
...
const odds = (await (await fetch(`${API}/admin/packs/${SLUG}/odds`, { headers: AH })).json()).odds;
const topId = odds[0].card_id;
const baselinePct = odds[0].pct;
await fetch(`${API}/admin/packs/${SLUG}/odds`, {
  method: "POST", headers: AH,
  body: JSON.stringify({ entries: odds.map((o, i) => ({ card_id: o.card_id, locked: i === 0, pct: i === 0 ? 40 : 0 })) }),
});
...
const regTok = (await (await fetch(`${API}/auth/customer/emailpass/register`, {
  method: "POST", headers: SH, body: JSON.stringify({ email, password }),
})).json()).token;
await fetch(`${API}/store/customers`, {
  method: "POST", headers: { ...SH, Authorization: `Bearer ${regTok}` }, body: JSON.stringify({ email }),
});
const loginTok = (await (await fetch(`${API}/auth/customer/emailpass`, {
  method: "POST", headers: SH, body: JSON.stringify({ email, password }),
})).json()).token;
const CH = { ...SH, Authorization: `Bearer ${loginTok}` };
const meId = (await (await fetch(`${API}/store/customers/me`, { headers: CH })).json()).customer.id;

// --- open N times, tally the won card ---
const tally = {};
let opened = 0;
for (let i = 0; i < N; i++) {
  const res = await j(await fetch(`${API}/store/packs/${SLUG}/open`, { method: "POST", headers: CH }));
...
execSync(`docker exec pokenic-postgres psql -U medusa -d medusa -c "DELETE FROM pull WHERE customer_id = '${meId}';"`, { stdio: "pipe" });
execSync(
...
out.verdict = out.withinBand && opened === N ? "PASS (roll shifted to the saved win rate)" : "FAIL";
