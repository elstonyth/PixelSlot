...
const ROUTES = ["/marketplace", "/leaderboard", "/how-it-works", "/pack-party", "/claw"];
...
for (const route of ROUTES) {
  ...
  for (const [site, origin] of Object.entries(ORIGIN)) {
    ...
    try {
      ...
      for (let i = 0; i < 20; i++) { const r = await page.evaluate(() => document.images.length > 2).catch(() => false); if (r) break; await page.waitForTimeout(500); }
      await page.waitForTimeout(1800);
      await page.screenshot({ path: `${OUT}/${site}_${slug}.png` });
      log.push(`${site.padEnd(6)} ${route.padEnd(16)} http=${resp ? resp.status() : "?"}`);
    } catch (e) {
