...
const EXTRACT = () => {
  ...
  const h1s = [...document.querySelectorAll("h1")].filter((h) => h.getBoundingClientRect().top < 700);
  const h1 = h1s[0] || null;
  ...
  for (let i = 0; i < 4 && hero && hero.parentElement; i++) {
    if (hero.querySelector("button, a[role=button]") && hero.querySelector("img")) break;
  ...
  const heading = h1
    ? {
        ...
        styles: cs(h1, ["fontSize", "fontWeight", "fontFamily", "lineHeight", "letterSpacing", "color", "textTransform"]),
        rect: rect(h1),
  ...
  const buttons = [...scope.querySelectorAll("button, a")]
    .filter((b) => b.getBoundingClientRect().width > 40 && b.getBoundingClientRect().top < 700 && b.textContent.trim())
    .slice(0, 6)
    .map((b) => ({
      tag: b.tagName.toLowerCase(),
      text: b.textContent.trim().slice(0, 30),
      styles: cs(b, ["backgroundColor", "color", "borderRadius", "padding", "fontSize", "fontWeight", "border"]),
      rect: rect(b),
    }));
  ...
  const pillCands = [...scope.querySelectorAll("*")]
    .filter((el) => {
      ...
      return r.height > 12 && r.height < 48 && r.width > 40 && r.width < 360 && rad >= 8 &&
        (h1 ? r.top <= h1.getBoundingClientRect().top + 4 : r.top < 300) && el.textContent.trim().length > 0 && el.textContent.trim().length < 40;
    })
    .slice(0, 4)
    .map((el) => ({
      text: el.textContent.trim().slice(0, 40),
      styles: cs(el, ["backgroundColor", "color", "borderRadius", "padding", "fontSize", "border"]),
      rect: rect(el),
    }));

  // Largest image in hero scope
  const imgs = [...scope.querySelectorAll("img")]
    .map((im) => ({ el: im, area: im.getBoundingClientRect().width * im.getBoundingClientRect().height }))
    .sort((a, b) => b.area - a.area);
  const bigImg = imgs[0]
    ? {
        src: (imgs[0].el.currentSrc || imgs[0].el.src).slice(-90),
        natural: { w: imgs[0].el.naturalWidth, h: imgs[0].el.naturalHeight },
        styles: cs(imgs[0].el, ["objectFit", "objectPosition", "width", "height", "borderRadius", "filter", "opacity"]),
        rect: rect(imgs[0].el),
      }
    : null;
...
for (const [url, site] of SITES) {
  ...
  try {
    ...
    for (let i = 0; i < 25; i++) {
      const ready = await page.evaluate(() => document.querySelector("h1") && document.images.length > 2).catch(() => false);
      if (ready) break;
