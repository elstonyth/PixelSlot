...
const CROP = 0.30; // top fraction (banner zone)
const browser = await chromium.launch();
const page = await browser.newPage({ viewport: { width: 1100, height: 1200 }, deviceScaleFactor: 2 });

for (const base of BASES) {
  const origB = "data:image/png;base64," + (await readFile(`docs/research/packdetail/lama-in/${base}.png`)).toString("base64");
  const newB = "data:image/webp;base64," + (await readFile(`public/images/claw/${base}-machine.webp`)).toString("base64");
  ...
  const f = resolve(`docs/research/packdetail/_cmp_${base}.html`).replace(/\\/g, "/");
