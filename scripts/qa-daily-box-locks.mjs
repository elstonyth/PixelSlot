...
//
// ponytail: the restore snapshot is held in memory only — if a run crashes
// between author and restore, the NEXT run snapshots the leftover QA state and
// "restores" that. After a crashed run, re-author tier a by hand (the
// admin_action_audit trail has the pre-test state's reason lines).
...
// --- restore tier a to its pre-test state ---
  ...
  body: JSON.stringify({
    ...
    prizes: before.prizes.map((p) => ({
      ...
      ...(p.kind === 'credit' || p.kind === 'voucher'
        ...
        : {}),
      ...(p.kind === 'product'
        ? { product_handle: p.payload.product_handle, qty: p.payload.qty ?? 1 }
        : {}),
    })),
  }),
});
