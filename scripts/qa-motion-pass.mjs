...
const cylRotY = (page) =>
  page.evaluate(() => {
    ...
    if (tr.startsWith("matrix3d")) return (Math.atan2(m[8], m[0]) * 180) / Math.PI;
    return 0;
  });
...
try {
  ...
  pass("drag rotates the cylinder", Math.abs(during) > 20, `rotY≈${during.toFixed(1)}° during drag`);
  ...
  pass("release snaps to a 60° slot", Math.abs(during) > 20 && snapErr < 2, `settled ${settled.toFixed(1)}° (err ${snapErr.toFixed(1)}°)`);

  // 1c. shuffle spins
  await dialog.getByRole("button", { name: /Shuffle/i }).click();
  await page.waitForTimeout(1400);
  const afterShuffle = (await cylRotY(page)) ?? 0;
  pass("shuffle spins the cylinder", Math.abs(afterShuffle - settled) > 30, `${settled.toFixed(0)}° -> ${afterShuffle.toFixed(0)}°`);

  // 2. select-tap -> slab holds (no instant skip to metadata)
  await page.mouse.click(720, 450);
  await page.waitForTimeout(700); // packs still exiting / slab rising
  const earlyMeta = await dialog.getByText(/^(Year|Value|Grade)$/).first().isVisible().catch(() => false);
  pass("select-tap does not skip the slab stage", !earlyMeta);
  await page.waitForTimeout(900);
  pass("slab visible (Tap to reveal)", await dialog.getByText(/Tap to reveal/i).first().isVisible().catch(() => false));

  // 3. advance: slab -> metadata -> (pull?) -> card via taps
  await page.mouse.click(720, 500);
  await page.waitForTimeout(400);
  pass("metadata stage shows (Year/Value row)", await dialog.getByText(/^(Year|Value)$/).first().isVisible().catch(() => false));
  await page.mouse.click(720, 500);
  await page.waitForTimeout(300);
  await page.mouse.click(720, 500); // in case a pull stage was in between
  await page.waitForTimeout(600);
  pass("card stage reached (Continue)", await dialog.getByRole("button", { name: /^Continue$/ }).first().isVisible().catch(() => false));
  const imgOk = await page.evaluate(() => [...document.querySelectorAll("img")].some((i) => i.src.includes("/cdn/cards/") && i.naturalWidth > 50));
  ...
  await rpage.goto(`${BASE}/claw/pokemon-mythic`, { waitUntil: "networkidle", timeout: 60000 });
  await rpage.getByRole("button", { name: /Try a free demo spin/i }).click();
  await rpage.waitForTimeout(500);
  pass("reduced-motion: jumps straight to the card", await rpage.getByRole("button", { name: /^Continue$/ }).first().isVisible().catch(() => false));
  pass("reduced-motion: no cylinder stage", !(await rpage.getByText(/Drag to spin/i).first().isVisible().catch(() => false)));
  ...
  const snap = () =>
    rpage.evaluate(() =>
      [...document.querySelectorAll("img")]
        .filter((im) => { const r = im.getBoundingClientRect(); return r.top < 560 && r.x > 560 && r.width > 50; })
        .map((im) => { let w = im.parentElement, d = 0; while (w && d < 4 && getComputedStyle(w).opacity === "1" && getComputedStyle(w).transform === "none") { w = w.parentElement; d++; } return `${(im.src || "").split("/").pop()}:${getComputedStyle(w).opacity}`; })
        .sort()
        .join("|"));
  const a = await snap();
