...
for (let i=0;i<25;i++){ if(await page.evaluate(()=>document.querySelectorAll('img').length>5)) break; await page.waitForTimeout(1000);} 
...
const meta = [];
for (let f = 0; f < 80; f++) {
  await page.screenshot({ path: `${OUT}/frame_${String(f).padStart(3,"0")}.png`, clip });
  ...
  const m = await page.evaluate(() => {
    const imgs = [...document.querySelectorAll("img")].filter(im => {
      ...
      return r.top < 560 && r.bottom > 40 && r.x > 560 && r.width > 50 && r.height > 50;
    }).map(im => {
      const cs = getComputedStyle(im);
      const r = im.getBoundingClientRect();
      // climb to the positioned ancestor that carries the transform/opacity (the slot wrapper)
      let wrap = im.parentElement, wcs = getComputedStyle(wrap), depth=0;
      while (wrap && depth<4 && wcs.transform === "none" && wcs.opacity === "1") { wrap = wrap.parentElement; wcs = getComputedStyle(wrap); depth++; }
      return {
        src: (im.currentSrc||im.src||"").split("/").pop().split("?")[0],
        cx: Math.round(r.x + r.width/2), w: Math.round(r.width), h: Math.round(r.height),
        op: +(+cs.opacity).toFixed(2),
        wrapOp: +(+wcs.opacity).toFixed(2),
        wrapT: wcs.transform === "none" ? "" : wcs.transform,
        filter: cs.filter === "none" ? (wcs.filter === "none" ? "" : wcs.filter) : cs.filter,
      };
    });
    // background glow: any element with a radial-gradient or large blur near hero
    const glow = [...document.querySelectorAll("*")].filter(el => {
      const cs = getComputedStyle(el); const r = el.getBoundingClientRect();
      return r.top < 560 && r.width > 300 && (/radial-gradient|linear-gradient/.test(cs.backgroundImage) || parseFloat(cs.filter.replace(/[^\d.]/g,"")) > 20 || cs.filter.includes("blur"));
    }).slice(0,3).map(el => { const cs=getComputedStyle(el); return { bg: (cs.backgroundImage||"").slice(0,80), filter: cs.filter, op:+(+cs.opacity).toFixed(2) }; });
    return { imgs, glow };
  });
  meta.push({ f, ...m });
...
const lines = meta.map(s => {
  const cards = s.imgs.filter(i => i.src.includes("ripped-packs") || i.src.includes("slabs"));
  // group by theme (strip slab/pack distinction)
  const byTheme = {};
  cards.forEach(c => { const t = c.src.replace(/\.webp$/,"").replace(/[0-9]+$/,""); (byTheme[t] ||= []).push(c); });
