...
const sel = await p.evaluate(()=>{
  const o=[...document.querySelectorAll('[class*="group/card"]')].find(e=>getComputedStyle(e).pointerEvents==="auto");
  o.setAttribute("data-test-card","1");
  return "[data-test-card]";
});
const before = await p.evaluate(()=>getComputedStyle(document.querySelector("[data-test-card]").firstElementChild).transform);
...
const after = await p.evaluate(()=>getComputedStyle(document.querySelector("[data-test-card]").firstElementChild).transform);
console.log("before :hover ->", before);
console.log("after  :hover ->", after);
console.log("RESULT:", before!==after && after!=="none" ? "PASS — group-hover/card lift works" : "still none");
await b.close();
