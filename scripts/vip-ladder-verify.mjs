...
ok('10 decade groups rendered', (await details.count()) === 10, String(await details.count()));
...
ok('row inputs become visible', await page.getByLabel('Level 5 threshold').isVisible());
...
ok('row inputs hidden when collapsed', !(await page.getByLabel('Level 5 threshold').isVisible()));
...
ok('only the erroring decade (61-70) stays open', JSON.stringify(openKeys) ===
  JSON.stringify([false, false, false, false, false, false, true, false, false, false]),
  JSON.stringify(openKeys));
ok('error is visible', await page.getByText(/Level 63: threshold must exceed/).isVisible());
...
ok('save payload carries all 100 levels', payload?.levels?.length === 100, JSON.stringify(payload?.levels?.length));
ok('levels are 1..100 in order',
  JSON.stringify(payload?.levels?.map((l) => l.level)) ===
    JSON.stringify(Array.from({ length: 100 }, (_, i) => i + 1)));
ok('reason forwarded', payload?.reason === 'phase B verification', String(payload?.reason));
