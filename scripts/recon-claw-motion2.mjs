...
const found = await page.evaluate(() => {
  ...
  const walk = (el, depth) => {
    ...
    dump.push({ depth, tag: el.tagName, cls: (el.className || "").toString().slice(0, 70), w: Math.round(r.width), h: Math.round(r.height), src: (el.currentSrc || el.getAttribute?.("src") || "").slice(0, 70), anim: cs.animationName, tf: cs.transform === "none" ? "" : cs.transform });
...
for (let t = 0; t < 24; t++) {
  const snap = await page.evaluate(() => {
    ...
    let i = 0;
    for (const el of all) { const r = el.getBoundingClientRect(); arr.push([i++, Math.round(r.x * 10) / 10, Math.round(r.y * 10) / 10]); }
    return arr;
  });
  series.push(snap);
...
for (let i = 0; i < n; i++) {
  ...
  if (xr > 1.5 || yr > 1.5) movers.push({ index: i, xRange: +xr.toFixed(1), yRange: +yr.toFixed(1) });
}
writeFileSync(`${OUT}/recon-movers.json`, JSON.stringify({ totalTracked: n, movers }, null, 2));
