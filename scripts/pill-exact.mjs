...
const out = await p.evaluate(()=>{
  ...
  return pills.map(pl=>{
    const label=(pl.querySelector(".font-semibold")?.textContent||"").trim();
    const svgClasses=[...pl.querySelectorAll(":scope svg")].map(s=>(s.getAttribute("class")||"").replace("lucide lucide-","")).join(" + ");
