...
const home = await page.evaluate(() => {
  const eyebrow = [...document.querySelectorAll("p")].find((e) => e.textContent.trim() === "Packs available now");
  const hiw = [...document.querySelectorAll("a")].find((a) => a.textContent.trim() === "How it works" && a.getBoundingClientRect().width > 0);
  return {
    eyebrowColor: eyebrow ? getComputedStyle(eyebrow).color : "NOT FOUND",
    hiwFont: hiw ? getComputedStyle(hiw).fontSize + " / " + getComputedStyle(hiw).fontWeight : "NOT FOUND",
  };
});
...
await page.goto(BASE + "/claw", { waitUntil: "domcontentloaded", timeout: 60000 });
await page.waitForTimeout(1800);
const claw = await page.evaluate(() => {
  const opens = [...document.querySelectorAll("a")].filter((e) => e.textContent.trim().toLowerCase() === "open");
  let grid = null, node = opens[0];
  for (let i = 0; i < 8 && node?.parentElement; i++) {
    const p = node.parentElement;
    if ([...p.children].filter((c) => c.querySelector("a")).length >= 4) { grid = p; break; }
    node = p;
  }
...
report.push(`CLAW pack columns @1440 (want 5): ${claw.columns}, cardW=${claw.cardWidth}`);
...
const lb = await page.evaluate(() => {
  const vol = [...document.querySelectorAll("td")].map((t) => t.textContent.trim()).find((t) => /\$\d/.test(t));
  const hasHeading = [...document.querySelectorAll("h2")].some((h) => h.textContent.trim() === "Weekly Leaderboard");
  return { vol: vol || "?", hasWeeklyHeading: hasHeading };
});
report.push(`LEADERBOARD volume sample (want US$...): ${lb.vol}`);
report.push(`LEADERBOARD has 'Weekly Leaderboard' heading (want false): ${lb.hasWeeklyHeading}`);
...
for (const route of ["/", "/claw", "/leaderboard"]) {
  for (const w of WIDTHS) {
    ...
    try {
      ...
      report.push(`${route.padEnd(13)} ${String(w).padStart(4)}px  overflow=${String(m.overflow).padStart(4)}${flag}${bflag}`);
    } catch (e) { report.push(`${route} ${w} FAIL ${e.message}`); }
    await c.close();
  }
}
