...
const pills = await p.evaluate(()=>{
  ...
  const pills=[...sec.querySelectorAll("a,div")].filter(e=>/View all packs|85-90% instant cash back|Ships worldwide/.test(e.textContent||"") && (e.className||"").includes("rounded-xl") && e.querySelector("svg,img"));
  return pills.slice(0,3).map(pl=>{
    const label=(pl.querySelector(".font-semibold")?.textContent||pl.textContent||"").trim().slice(0,30);
    const tag=pl.tagName;
    const imgs=pl.querySelectorAll("img").length;
    const svgs=[...pl.querySelectorAll("svg")].map(s=>s.getAttribute("class")||"").join(",");
    const isLink=tag==="A";
