...
// pack-party: count infinite animations (ping + gradientShift) + live dots
{
  ...
  await page.goto(BASE + "/pack-party", { waitUntil: "domcontentloaded", timeout: 60000 });
  await page.waitForTimeout(2000);
  const anim = await page.evaluate(() => {
    const m = {};
    for (const el of document.querySelectorAll("*")) {
      const s = getComputedStyle(el);
      if (s.animationName && s.animationName !== "none" && s.animationIterationCount === "infinite") m[s.animationName] = (m[s.animationName] || 0) + 1;
    }
    return m;
  });
...
// how-it-works: video autoplay attr
{
  ...
  const v = await page.evaluate(() => {
    ...
    return vid ? { src: vid.getAttribute("src"), autoplay: vid.autoplay, loop: vid.loop, muted: vid.muted, playsInline: vid.playsInline } : "NO VIDEO";
  });
...
for (const route of ["/pack-party", "/claw/pokemon-mythic", "/claw"]) {
  for (const w of WIDTHS) {
    ...
    try {
      ...
      const m = await p.evaluate(() => ({ o: document.documentElement.scrollWidth - window.innerWidth, b: [...document.querySelectorAll("img")].filter(i => i.complete && i.naturalWidth === 0).length }));
      report.push(`${route.padEnd(22)} ${String(w).padStart(4)}px  overflow=${String(m.o).padStart(4)}${m.o > 1 ? "  ⚠" : ""}${m.b ? "  ⚠" + m.b + "broken" : ""}`);
...
// capture mythic pack again
{
  ...
  const page = await ctx.newPage();
  await page.goto(BASE + "/claw/pokemon-mythic", { waitUntil: "domcontentloaded", timeout: 60000 });
  await page.waitForTimeout(2000);
