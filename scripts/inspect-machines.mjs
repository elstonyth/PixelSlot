...
const bases = [...new Set(files.filter((f) => /-machine\.(avif|webp)$/.test(f)).map((f) => f.replace(/-machine\.(avif|webp)$/, "")))].sort();
...
await page.screenshot({ path: "docs/research/packdetail/machines-grid.png", fullPage: true });
await browser.close();
