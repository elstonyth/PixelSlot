...
await page.evaluate(async () => { try { await document.fonts.load("700 56px Poppins"); } catch {} });

const results = await page.evaluate(async (PRODUCT) => {
  const load = (src) => new Promise((ok, no) => { const im = new Image(); im.onload = () => ok(im); im.onerror = () => no(new Error(src)); im.src = src; });
  const out = {};
  for (const base of PRODUCT) {
    let img;
    try { img = await load(`/images/claw/${base}-machine.avif`); }
    catch { try { img = await load(`/images/claw/${base}-machine.webp`); } catch { out[base] = { err: "load" }; continue; } }
    if (img.naturalWidth !== 1440 || img.naturalHeight !== 1000) { out[base] = { skip: img.naturalWidth + "x" + img.naturalHeight }; continue; }

    const cv = document.createElement("canvas"); cv.width = 1440; cv.height = 1000;
    const ctx = cv.getContext("2d"); ctx.drawImage(img, 0, 0, 1440, 1000);
    const id = ctx.getImageData(0, 0, 1440, 1000); const d = id.data;
    const px = (x, y) => { const i = (y * 1440 + x) * 4; return [d[i], d[i + 1], d[i + 2]]; };

    // "phygitals" brand line bbox (validated visually): x 30–58.5%, y 14.7–21.3%
    const x0 = 432, x1 = 842, y0 = 147, y1 = 213;
    ...
    const isText = (x, y) => { const i = (y * 1440 + x) * 4; return (d[i + 2] - d[i + 1]) > 8 && d[i + 2] > 115; };
    const M = Math.round(0.04 * 1440);
    for (let y = y0; y <= y1; y++) {
      for (let x = x0; x <= x1; x++) {
        if (!isText(x, y)) continue;
        let lc = null; for (let xx = x - 1; xx >= x0 - M; xx--) if (!isText(xx, y)) { const i = (y * 1440 + xx) * 4; lc = [d[i], d[i + 1], d[i + 2]]; break; }
        let rc = null; for (let xx = x + 1; xx <= x1 + M; xx++) if (!isText(xx, y)) { const i = (y * 1440 + xx) * 4; rc = [d[i], d[i + 1], d[i + 2]]; break; }
        const col = lc && rc ? [(lc[0] + rc[0]) / 2, (lc[1] + rc[1]) / 2, (lc[2] + rc[2]) / 2] : (lc || rc || [214, 215, 218]);
        const i = (y * 1440 + x) * 4; d[i] = col[0]; d[i + 1] = col[1]; d[i + 2] = col[2]; d[i + 3] = 255;
      }
    }
    ...
    out[base] = { ok: true, purple, fs, data: cv.toDataURL("image/webp", 0.95) };
  }
  return out;
}, PRODUCT);
...
for (const [base, r] of Object.entries(results)) {
  if (r.ok) { await writeFile(`public/images/claw/${base}-machine.webp`, Buffer.from(r.data.split(",")[1], "base64")); console.log(`${base.padEnd(30)} edited  purple=${r.purple} fs=${r.fs}`); edited++; }
