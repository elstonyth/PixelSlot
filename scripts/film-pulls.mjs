...
const SITES = { ORIG: "https://www.phygitals.com/", CLONE: "http://localhost:4000/" };

const SCROLL_TO_PULLS = () => {
  const h = [...document.querySelectorAll("h2,h3,p")].find((e) => /recent pulls/i.test(e.textContent));
...
const FIRST_CARD_X = () => {
  ...
  for (let up = 0; up < 5 && node; up++, node = node.parentElement) {
    const cand = [...node.querySelectorAll("div")].find((d) => {
      ...
      return s.display === "flex" && d.children.length >= 4 && d.getBoundingClientRect().width > 600;
    });
...
for (const [name, url] of Object.entries(SITES)) {
  ...
  try {
    ...
    for (let i = 0; i < 20; i++) { const r = await page.evaluate(() => document.images.length > 3).catch(() => false); if (r) break; await page.waitForTimeout(500); }
    ...
    console.log(`${name}: x0=${x0}  x1=${x1}  movedPx=${moved}  ${typeof moved === "number" && moved > 2 ? "→ AUTO-SCROLLS" : "→ static"}`);
