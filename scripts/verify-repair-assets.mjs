...
for (const f of fs.readdirSync(COMP).filter((f) => f.endsWith(".tsx"))) {
  ...
  for (const m of s.matchAll(/(\/[A-Za-z0-9_\/.-]*\.(?:webp|png|jpg|jpeg))/g)) refs.add(m[1]);
}
...
function remoteFor(p) {
  ...
  return "https://www.phygitals.com" + p.replace(/(\/social\/tweets\/\d+)_media-1\.webp/, "$1:media-1.webp");
}
...
function pngStatus(buf) {
  ...
  if (buf.subarray(buf.length - 8).toString("ascii", 0, 4) !== "IEND") return "no-IEND (truncated)";
  return "ok";
}
function jpgStatus(buf) {
  if (buf.length < 4 || buf[0] !== 0xff || buf[1] !== 0xd8) return "bad-jpg-header";
  if (buf[buf.length - 2] !== 0xff || buf[buf.length - 1] !== 0xd9) return "no-EOI (truncated)";
  return "ok";
}
...
console.log(`Validated ${refs.size} referenced images: ${okCount} ok, ${bad.length} bad`);
...
for (const b of bad) {
  ...
  try {
    ...
    if (!res.ok) { console.log(`  FAIL ${res.status} ${b.p}`); failed++; continue; }
    const buf = Buffer.from(await res.arrayBuffer());
    const st = statusFor(path.basename(b.p), buf);
    if (st !== "ok") { console.log(`  STILL-BAD [${st}] ${b.p}`); failed++; continue; }
