...
async function getEls(){
  return p.evaluateHandle(()=>{
    const outer=[...document.querySelectorAll('[class*="group/card"]')].find(e=>getComputedStyle(e).pointerEvents==="auto");
    return outer;
  });
}
// read transform of inner child of current center card
async function innerTf(){return p.evaluate(()=>{const o=[...document.querySelectorAll('[class*="group/card"]')].find(e=>getComputedStyle(e).pointerEvents==="auto");return o?getComputedStyle(o.firstElementChild).transform:"none";});}
...
console.log("RESULT:", (offText==="none" && onCard!=="none") ? "PASS (card lifts only on card hover)" : (offText!==onCard? "PARTIAL (differs)" : "FAIL (no change)"));
await b.close();
