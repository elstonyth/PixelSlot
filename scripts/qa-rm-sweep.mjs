...
for (const path of PAGES) {
  try {
    await p.goto(`http://localhost:4000${path}`, { waitUntil: 'networkidle', timeout: 30000 });
  } catch {
    // page may not exist — skip gracefully
  }
  const dollars = await p.$$eval('body *:not(script):not(style)', (els) =>
    els.filter((e) => e.children.length === 0 && /\$\s?\d/.test(e.textContent || '')).map((e) => e.textContent.trim()).slice(0, 20),
  );
