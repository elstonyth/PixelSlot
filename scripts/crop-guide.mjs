...
for (const a of args) {
  ...
  const data = "data:image/webp;base64," + (await readFile(`public/images/claw/${base}-machine.webp`)).toString("base64");
  ...
  await page.screenshot({ path: `docs/research/packdetail/guide_${base}.png`, clip: { x: box.x, y: box.y, width: box.width, height: box.height * 0.30 } });
