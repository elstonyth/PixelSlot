...
if (!found) { console.log("HIW NOT FOUND"); await b.close(); process.exit(0); }
...
const cardInfo = await p.evaluate(() => {
  ...
  const grids = [...sec.querySelectorAll("div")].filter(d => {
    const s = getComputedStyle(d); return s.display === "grid" || s.display === "flex";
  });
  // pick the container with 3+ children that look like cards
  let cards = [];
  for (const g of grids) {
    const kids = [...g.children].filter(c => c.textContent.trim().length > 10);
    if (kids.length >= 2 && kids.length <= 5) { cards = kids; break; }
  }
  cards.forEach((c, i) => c.setAttribute("data-hiwcard", String(i)));
  return { count: cards.length, transitions: cards.map(c => { const s = getComputedStyle(c); return { transition: s.transition, transform: s.transform, opacity: s.opacity }; }) };
});
...
for (let step = 0; step <= 30; step++) {
  ...
  const snap = await p.evaluate(() => {
    ...
    return cards.map(c => { const s = getComputedStyle(c); const r = c.getBoundingClientRect(); return { o: +(+s.opacity).toFixed(2), tf: s.transform === "none" ? "" : s.transform, top: Math.round(r.top) }; });
  });
...
samples.forEach(s => {
  ...
  console.log("s"+String(s.step).padStart(2)+" y"+s.scrollY+": "+parts.join(" | "));
});
await b.close();
