...
const EXTRACT = () => {
  ...
  const P = ["backgroundColor","backgroundImage","color","borderRadius","padding","fontSize","fontWeight","letterSpacing","textTransform","border","boxShadow"];

  // exact innermost element with this trimmed text
  const exact = (txt) => {
    const all = [...document.querySelectorAll("button,a,span,div,p")].filter((e) => e.textContent.trim().toLowerCase() === txt.toLowerCase());
  ...
  return {
    openPacks: cta ? { tag: cta.tagName.toLowerCase(), styles: cs(cta, P), rect: rct(cta) } : "NOT FOUND",
    eyebrow: eyebrow ? { tag: eyebrow.tagName.toLowerCase(), text: eyebrow.textContent.trim(), styles: cs(eyebrow, P), rect: rct(eyebrow) } : "NOT FOUND",
  };
};
