...
const dec2d = (m) => ({ sc: +Math.hypot(m[0], m[1]).toFixed(4), rot: +((Math.atan2(m[1], m[0]) * 180) / Math.PI).toFixed(2), tx: +m[4].toFixed(1), ty: +m[5].toFixed(1) });
const dec3d = (m) => ({ sc: +Math.hypot(m[0], m[1], m[2]).toFixed(4), rotY: +((Math.atan2(m[8], m[0]) * 180) / Math.PI).toFixed(2), tx: +m[12].toFixed(1), ty: +m[13].toFixed(1) });
const parseT = (tr) => {
  ...
  for (const [name, series] of themes) {
    ...
    while (i < series.length - 1) {
      ...
      if (seg.length > 4) {
        ...
        segs.push(`t=${Math.round(from.t)} +${Math.round(to.t - from.t)}ms cx ${from.cx}->${to.cx} wo ${from.wo}->${to.wo} sc ${from.sc}->${to.sc} ${ease}`);
      }
      i = j + 1;
    }
...
if (rev?.frames?.length) {
  ...
  for (const fr of rev.frames) {
    ...
    else if (/PULL •|PULL!|! PULL/.test(txt) || (/PULL/.test(txt) && /•/.test(txt))) stage = "pull";
    else if (/CONTINUE|OPEN ANOTHER|SELL|ADD TO/.test(txt)) stage = "card";
    else if (/CATEGORY|GRADE|YEAR|\b(19|20)\d\d\b/.test(txt)) stage = "metadata";
    if (stage && stage !== last) { P(`- STAGE ${String(Math.round(fr.t)).padStart(6)}ms  ${stage.padEnd(9)} "${(fr.txt || "").slice(0, 90)}"`); last = stage; }
  }
  // motion segments (same approach as pass 1, inline)
  const bySig = new Map();
  for (const fr of rev.frames) for (const n of fr.n) {
    const d = parseT(n.tr);
    (bySig.get(n.s) || bySig.set(n.s, []).get(n.s)).push({ t: fr.t, o: n.o, ...d, an: n.an, bg: n.bg, y: n.y, x: n.x, w: n.w, h: n.h });
  }
  P("### Motion segments:");
  const rows = [];
  for (const [sig, series] of bySig) {
    ...
    for (const prop of ["o", "sc", "rot", "rotY", "tx", "ty"]) {
      ...
      while (i < vals.length - 1) {
        ...
        const from = seg[0].v, to = seg.at(-1).v, delta = Math.abs(to - from);
        if (delta >= (prop === "o" ? 0.08 : 3)) {
          const t0 = seg[0].t, dur = seg.at(-1).t - t0;
          if (dur > 30 && dur < 5000) {
            const span = to - from || 1;
            const ease = fitEase(seg.map((s) => ({ p: (s.t - t0) / (dur || 1), v: (s.v - from) / span })));
  ...
  rows.sort((a, b) => a.t0 - b.t0).forEach((s) =>
    P(`- t=${String(Math.round(s.t0)).padStart(6)} +${String(Math.round(s.dur)).padStart(4)}ms ${s.prop.padEnd(4)} ${String(+s.from.toFixed(2)).padStart(8)} -> ${String(+s.to.toFixed(2)).padEnd(8)} ${s.ease.padEnd(26)} ${s.sig.slice(0, 58)}`));
  const anims = new Set();
  rev.frames.forEach((fr) => fr.n.forEach((n) => n.an && anims.add(`${n.an}  [${n.s.slice(0, 46)}]`)));
  const bgs = new Set();
  rev.frames.forEach((fr) => fr.n.forEach((n) => { if (n.bg && n.bg !== "rgba(0, 0, 0, 0)" && (n.h < 220 && n.w > 800)) bgs.add(`${n.bg} ${n.w}x${n.h} [${n.s.slice(0, 40)}]`); }));
  if (anims.size) { P("### CSS animations seen:"); [...anims].slice(0, 24).forEach((a) => P("- " + a)); }
  if (bgs.size) { P("### Wide colored bars (ribbon candidates):"); [...bgs].slice(0, 12).forEach((b) => P("- " + b)); }
} else P("\n## REVEAL v2 — no data");
