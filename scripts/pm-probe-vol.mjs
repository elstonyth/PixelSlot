...
const EXTRACT = () => {
  ...
  const cells = [...document.querySelectorAll("td,span,div")]
    .filter((e) => e.children.length === 0 && /\$\s?\d[\d,]*/.test(e.textContent))
...
for (const [site, origin] of Object.entries(ORIGIN)) {
  ...
  try {
    ...
    for (let i = 0; i < 22; i++) { const r = await page.evaluate(() => document.querySelectorAll("td").length > 0 || document.images.length > 4).catch(() => false); if (r) break; await page.waitForTimeout(500); }
