...
async function testPage(url, label, sizes) {
  for (const [w,h] of sizes) {
    ...
    const before = await p.evaluate(() => {
      const els = [...document.querySelectorAll("[style*='translateY'], [class*='opacity-0']")];
    ...
    await p.evaluate(async () => {
      ...
      for (let y=0; y<=h; y+=Math.round(window.innerHeight*0.6)) { window.scrollTo(0,y); await new Promise(r=>setTimeout(r,120)); }
      window.scrollTo(0, h);
      await new Promise(r=>setTimeout(r,900));
    });
    const after = await p.evaluate(() => {
      const stillHidden = [...document.querySelectorAll("*")].filter(e => +getComputedStyle(e).opacity < 0.05 && e.getBoundingClientRect().height > 20).length;
    ...
    console.log(`[${label} ${w}x${h}] belowFoldHiddenOnLoad=${before.belowFoldHidden} | afterScroll stillHidden=${after.stillHidden} broken=${after.broken} hOverflow=${after.overflowX}px`);
    await p.close();
  }
}

const sizes = [[390,844],[768,1024],[1440,900],[1920,1080],[2560,1440],[3840,2160]];
