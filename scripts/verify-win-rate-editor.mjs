...
// load current odds
const before = await (await fetch(`${API}/admin/packs/${SLUG}/odds`, { headers: AH })).json();
...
{
  const entries = rows.map((o, i) => ({ card_id: o.card_id, locked: i === 0, pct: i === 0 ? 40 : 0 }));
  const res = await fetch(`${API}/admin/packs/${SLUG}/odds`, {
    method: "POST", headers: AH, body: JSON.stringify({ entries }),
  });
  ...
  ok("save_locked_4000", top?.weight === 4000 && top?.locked === true, JSON.stringify(top));
  ok("save_even_split_400", others.every((o) => o.weight === 400), [...new Set(others.map((o) => o.weight))].join(","));

  // persisted?
  const after = await (await fetch(`${API}/admin/packs/${SLUG}/odds`, { headers: AH })).json();
  const topAfter = after.odds.find((o) => o.card_id === topId);
  ok("persisted_locked_40pct", topAfter?.locked === true && topAfter?.pct === 40, JSON.stringify(topAfter));
  const othersAfter = after.odds.filter((o) => o.card_id !== topId);
  ok("persisted_others_4pct", othersAfter.every((o) => o.pct === 4), [...new Set(othersAfter.map((o) => o.pct))].join(","));
}

// 4. no-leak after edit (customer surface)
{
  const res = await fetch(`${API}/store/packs/${SLUG}`, { headers: { "x-publishable-api-key": PK } });
  const j = await res.json();
  const entries = j.odds || [];
  ok("customer_no_weight", !entries.some((e) => "weight" in e), `keys: ${Object.keys(entries[0] || {})}`);
...
// 5. validation rejects
{
  ...
  const e1 = rows.map((o, i) => ({ card_id: o.card_id, locked: i < 2, pct: i === 0 ? 60 : i === 1 ? 50 : 0 }));
  const res1 = await fetch(`${API}/admin/packs/${SLUG}/odds`, { method: "POST", headers: AH, body: JSON.stringify({ entries: e1 }) });
  ...
  const res2 = await fetch(`${API}/admin/packs/${SLUG}/odds`, { method: "POST", headers: AH, body: JSON.stringify({ entries: e2 }) });
...
const restored = await (await fetch(`${API}/admin/packs/${SLUG}/odds`, { headers: AH })).json();
const topRestored = restored.odds.find((o) => o.card_id === topId);
ok("restored_seed", topRestored?.locked === false && topRestored?.pct !== 40, JSON.stringify(topRestored));
