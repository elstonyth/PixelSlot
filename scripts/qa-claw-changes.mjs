...
try {
  ...
  page.on("response", (r) => { if (r.status() >= 400) netFailures.push(`${r.status()} ${r.url().slice(0, 120)}`); });
  ...
  pass("P1 stepper − / + / MAX present",
    (await page.locator('button[aria-label="Decrease quantity"]').count()) > 0 &&
    (await page.locator('button[aria-label="Increase quantity"]').count()) > 0 &&
    (await page.getByRole("button", { name: "Max", exact: true }).count()) > 0);
  pass("P1 Open is a link (not a list real-open button)",
  ...
  pass("P2 boosted tiers show +90% Buyback Boost", await page.getByText("+90% Buyback Boost").first().isVisible().catch(() => false));
  pass("P2 Dragon Ball chip present", (await page.getByRole("button", { name: "Dragon Ball", exact: true }).count()) > 0);
  pass("P2 category rows are horizontal carousels", await page.evaluate(() =>
    [...document.querySelectorAll("section > div")].some((el) => el.className.includes("overflow-x-auto") && el.className.includes("flex"))));
  ...
  pass("P2 premium tiers show +92% Buyback Boost", await page.getByText("+92% Buyback Boost").first().isVisible().catch(() => false));
  pass("P2 out-of-stock tile present", await page.getByText(/Out of Stock/i).first().isVisible().catch(() => false));

  await page.getByRole("button", { name: "Dragon Ball", exact: true }).click();
  await page.waitForTimeout(400);
  pass("P2 Dragon Ball empty state", await page.getByText(/No packs available/i).first().isVisible().catch(() => false));
  ...
  const demo = await page.getByText(/demo spin/i).first().isVisible().catch(() => false);
  pass("P1 Open → detail (free demo spin, no login wall)", /\/claw\/[^/]+$/.test(page.url()) && demo);
  ...
  for (const w of [768, 375]) {
    ...
    pass(`responsive ${w}px: no horizontal overflow`, overflow <= 2, `overflow=${overflow}px`);
  }
  ...
  pass("regression: /repacks stepper present", (await page.locator('button[aria-label="Increase quantity"]').count()) > 0);
...
console.log(`console errors: ${realErrors.length}${realErrors.length ? "\n  - " + realErrors.slice(0, 8).join("\n  - ") : ""}`);
console.log(`network 4xx/5xx: ${realNet.length}${realNet.length ? "\n  - " + realNet.slice(0, 10).join("\n  - ") : ""}`);
console.log(`screenshots → ${OUT}/qa-claw-{1440,768,375,premium-oos,dragonball,detail}.png, qa-repacks-1440.png`);
